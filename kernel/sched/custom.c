...
static struct task_struct *pick_next_task_custom(struct rq *rq)
{
	...
	struct sched_custom_entity *next_se = list_first_entry(&rq->custom.task_list, struct sched_custom_entity, task_list);
	struct task_struct *next_task = container_of(next_se, struct task_struct, custom_se);
